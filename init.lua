...
local function tradeBook(bookName)
    if mq.TLO.Cursor() ~= nil then
        ...
        if mq.TLO.Cursor() ~= nil then
            print("Mendax: Cursor did not clear before trade.")
            return false
        end
    end
    ...
    if mq.TLO.Cursor() == nil then
        print("Mendax: Book did not appear on cursor.")
        return false
    end

    mq.cmd("/click left target")
    mq.delay(5000, function() return mq.TLO.Window("GiveWnd").Open() and not mq.TLO.Cursor() end)
    if not mq.TLO.Window("GiveWnd").Open() then
        print("Mendax: Trade window did not open.")
        return false
    end

    local giveBtn = mq.TLO.Window("GiveWnd").Child("GVW_Give_Button")
    if not giveBtn or not giveBtn() then
        print("Mendax: Give button not found.")
        return false
    end

    giveBtn.LeftMouseUp()
    mq.delay(5000, function() return not mq.TLO.Window("GiveWnd").Open() end)
    if mq.TLO.Window("GiveWnd").Open() then
        print("Mendax: Trade window did not close after give.")
        return false
    end
...
local function tradeAllBooks()
    for _, book in ipairs(books) do
        ...
        while mq.TLO.FindItemCount(bookString)() > 0 do
            ...
            if not tradeBook(bookItem.Name() or "") then
                print("Mendax: Trade failed! Exiting.")
                mq.exit()
            end
        end
    end
...
-- Main
print("Mendax: Starting Book Check...")
